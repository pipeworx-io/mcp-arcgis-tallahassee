 ...
 *
 * Tallahassee-Leon County GIS (ArcGIS org ptvDyBs1KkcwzQNJ): parcels, zoning, public works & planning. search_datasets finds layers (each
...
const UA = 'pipeworx-mcp-arcgis-tallahassee/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'search_datasets',
    description: 'Search Tallahassee-Leon County GIS open geospatial datasets (parcels, zoning, public works & planning) by keyword. Returns each dataset\'s name, summary, record_count, owner/org, and its Feature Service `url` — pass that url to query_layer / layer_info.',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Keyword(s), e.g. "parcels", "crime", "flood zones".' },
        org_id: { type: 'string', description: 'Optional ArcGIS orgId to override the default (Tallahassee-Leon County GIS).' },
